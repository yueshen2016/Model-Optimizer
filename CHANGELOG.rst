...
- Add an optional ``skip_methods`` argument to ``mto.enable_huggingface_checkpointing`` to leave specific patched methods (e.g. ``_from_config``) unpatched, as an escape hatch for custom models whose construction conflicts with ModelOpt's restore hook.
